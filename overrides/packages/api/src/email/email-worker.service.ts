...
export class EmailWorkerService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Email worker startup recovery did not complete: ${String(error).slice(0, 500)}`);
    }
    ...
    try {
      ...
      for (const row of rows) {
        ...
        try {
          ...
          this.logger.error(`Email outbox item ${row.id} did not deliver on attempt ${attempts}`);
        }
      }
      return delivered;
    } catch (error) {
      this.logger.error(`Email outbox batch was deferred without affecting the user request: ${String(error).slice(0, 500)}`);
      return 0;
    }
  }
}
